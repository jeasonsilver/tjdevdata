	...
	ligergrid =  $("#maingrid").ligerGrid({
            columns: 
				[ 
					...
					{display : '序号',name : 'ordercol',width:"120"},
					{display : '标题',name : 'bt',
						render:function(row){
            ...
            url: '${ctx}/dblz/cwhsy/ajax_list.do',
...
function f_edit(id){
  ...
  var newDialog = new Dialog({type:'iframe',value:'${ctx}/dblz/cwhsy/edit.do?id='+id},
	...
	**/
  window.open('${ctx}/dblz/cwhsy/edit.do?id='+id,"_blank");
}
...
function f_add(){
	...
	**/
	 window.open('${ctx}/dblz/cwhsy/input.do',"_blank");
}

function f_view(id){
	  var newDialog = new Dialog({type:'iframe',value:'${ctx}/dblz/cwhsy/view.do?id='+id},
					{title:'常委会审议',modal:true,width:1000,height:450}).show();
}
