		...
		$(function(){
				...
				$("#file_upload").uploadify({
			        ...
			        'onUploadSuccess':function(file, data, response){
			             $("#filename").html(file.name+"<a class='jq_fj' href='javascript:void(0);' onClick='deleteAccessory();' >删除</a>");
...
<form class="span12"  method="post" id="inputForm" action="${ctx}/dblz/lzjycg/save.do" >
	...
	<input type="hidden" name="ly" value="${ob.ly}" />
	...
	<div class="row" style="margin-top: 10px">
		...
		<label class="span10" style="padding-top: 4px;padding-bottom: 4px">
          	<c:if test="${ob.ly==1}">个人发布</c:if>
          	<c:if test="${ob.ly==2}">单位发布</c:if>
		</label>
		...
		<div class="span10" style="margin-top: 10px;white-space:nowrap">
			...
			<td><div id="filename" style="float: left;">${ob.filename}</div> </td>
			</tr>
			</table>
		</div>
