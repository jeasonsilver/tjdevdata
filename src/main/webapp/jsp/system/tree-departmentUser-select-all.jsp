    ...
    for (var i = 0; i < nodes.length; i++)
    {
        if (nodes[i].iconSkin){//只取用户节点
	        ...
	        $.ajax(
					{
			       		   type: "POST",
			       		   url: "${ctx}/dblz/representinfo/user_ajax.do",
			       		   data: {id:text},
			       		   success: function(data){
			       			$("#dbzh").val(data.dbzh);//代表证号
			       			$("#dbt").val(data.deptname);//代表团
			       			$("#dbtid").val(data.deptid);//代表团
			       			$("#mobile").val(data.yddh);//移动电话
			       			$("#address").val(data.dwdz);//单位通讯地址
			       			$("#yzbm").val(data.dwmb);//单位邮编
			       		}
			         }
			);
