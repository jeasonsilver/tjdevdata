...
function Dialog(content, options)
{
    var defaults = { // 默认值。 
        ...
        width:500,            // 
        height:300,            // 
        dialogCss:null         // 如果center为false,则调用这个css 例如{bottom:10,right:20}
    };
    
    //alert对话框
    if (content.type=="alert"|| content.type=="confirm"){
    	defaults.width=250;
    	defaults.height=80;
    }
    ...
    var isIe6 = false; //$.browser.msie && ('6.0' == $.browser.version);

    /* 对话框的布局及标题内容。*/
    var barHtml = !options.showTitle ? '' :
        '<div class="bar"><span class="title"><i class="icon-file icon-white"></i>' + options.title + '</span><button type="button" class="close" data-dismiss="alert">&times;</button></div>';
    var dialog = $('<div id="' + options.id + '"  class="dialog">'+barHtml+
    		'<div class="content" style="width:'+options.width+'px;height:'+options.height+'px;overflow:auto;"></div></div>').hide();
    $('body').append(dialog);
    ...
    var init = function()
    {
        ...
        //$("#"+overlayId).bind('click', this.close); //2014-11-5 modified by lijin 李晓琴秘书长提出一点遮罩就关闭录入内容没了
    ...
    this.setContent = function(c)
    {
        ...
        if('object' == typeof(c))
        {
            ...
            case 'url':
                ...
                break;
            case 'iframe':
                div.append($('<iframe src="' + c.value + '" frameborder="no"/>'));
                break;
            case 'confirm':
                div.append($('<div style="text-align:center"><div style="font-size:16px;height:50px">'+c.message+'</div><input type="button" class="btn btn-success ok" value="确定" >'
                		+'&nbsp;&nbsp;<input type="button" class="btn" id="close" value="取消" ></div>'));
                break;
            case 'alert':
                div.append($('<div style="text-align:center"><div style="font-size:16px;height:'+c.height+'">'+c.message+'</div><input type="button" class="btn btn-success ok" value="确定" >'
                		+'</div>'));
                break;
            case 'text':
            default:
...
//快速alert
function freeAlert(msg)
{
	var dlg = new Dialog({type:'alert',message:msg});
	dlg.show();
	return dlg;
}
//快速confirm
function freeConfirm(msg,fn)
{
	var dlg = new Dialog({type:'confirm',message:msg,fn:fn});
	dlg.show();
	return dlg;
}
//快速alert
function freeAlert(msg,fn)
{
	var dlg = new Dialog({type:'alert',message:msg,fn:fn});
	dlg.show();
	return dlg;
}
