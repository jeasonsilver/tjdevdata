...
function openjt(url,reportname){
	 //alert('11');
	...
	window.location.href="showgbasebichart.jsp?"+url+"&reportname="+encodeURI(reportname);
	}
//window.onload=
function init()
{
...
function createUL(data, obj)
{
    ...
    for(var i=0; i<num; i++){
    	 ...
    	 if(url.indexOf("fid")>=0){
    		...
    		//var t= Date.valueOf();//Date.now();
    		//alert(t);
    		 //url+="&t="+t;
    		//alert(url);
    		//strUl += "<li><a href='" + url + "'  target='_blank' >"+ lst[i]['menuname'] + "</a></li>";
    		strUl += "<li><a  href='#' onclick=openjt('"+url+"','"+lst[i]['menuname']+"')>"+ lst[i]['menuname'] + "</a></li>";
    		// alert(url);
    	 }else{
    		// strUl += "<li><a href='" + url + "'  target='_blank' >"+ lst[i]['menuname'] + "</a></li>";
...
<style>
	body {/*渐变背景*/
		background:  url(ui/images/login/bg.png); background-size: cover;-webkit-background-size: cover;-moz-background-size: cover;-o-background-size: cover;
		backgrond-attachment:fixed;background-position:center;
...
<body onload="init()">
	...
	<div style="height:100%;"><div class="nav_title">大数据平台<!--#999999 -->
	<ul class="nav_rig" style="margin:10px;color: white;">
	   
	    <li><span class="text_grey" ><font color="#2275B9">欢迎：&nbsp;<shiro:principal property="xm" /></font></span>
		     <a class="ico_01" href="${ctx}/index.jsp" target="_self" title="回到主页" ></a>
		    <a href="${ctx}/index.jsp" target="_self" style="color: #2275B9;"><span style="float: left;padding-top: 5px;">回到主页</span></a>
		   
		    <a class="ico_04" href="${ctx}/system/user/logout.do" title="退出系统" ></a>
		     <a href="${ctx}/system/user/logout.do" target="_self" style="color: #2275B9;"><span style="float: left;padding-top: 5px">退出系统</span></a>
		    
	    </li>
  	</ul>
  	</div>
  	 <div id="mainId" style="height:100%;">
